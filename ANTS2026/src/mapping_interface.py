...
class ButtonMapper(Pillar_Mapper_Base):
    """
    Maps each tube to a specific hardcoded synth.
    Triggers a new synth instance on every button press.
    """
    def __init__(self, cfg, pillar_cfg):
        super().__init__(cfg, pillar_cfg)
        self.tube_synths = pillar_cfg.get("tube_synths", [])
        # Fallback to a default if not enough synths provided
        if len(self.tube_synths) < self.num_tubes:
    ...
    def interaction_update_sound_light(self, old_state, new_state):
        ...
        for tube_id, (old_active, active) in enumerate(zip(old_state, new_state)):
            if not old_active and active:
                synth_name = self.tube_synths[tube_id]
                print(f"[MAPPER][ButtonMapper] tube={tube_id} -> synth='{synth_name}'")
                # We use reaction_notes to pass the synth name or a trigger to the sound manager
                # In this architecture, we'll append a tuple or dict if the manager supports it,
                # or we might need to adjust SoundState to handle per-button synths.
                # Assuming the manager handles 'reaction_notes' as triggers:
                self.sound_state.append_reaction_notes({"synth": synth_name, "tube_id": tube_id})
